...
function wp_bootstrap_starter_posted_on() {
	...
	$posted_on = sprintf(
		esc_html_x( 'Опубликовано %s', 'post date', 'wp-bootstrap-starter' ),
		'<a href="' . esc_url( get_permalink() ) . '" rel="bookmark">' . $time_string . '</a>'
	);

	$byline = sprintf(
		esc_html_x( 'Автор %s', 'post author', 'wp-bootstrap-starter' ),
...
function wp_bootstrap_starter_entry_footer() {
	...
	if ( 'post' === get_post_type() ) {
		...
		if ( $tags_list ) {
			printf( ' | <span class="tags-links">' . esc_html__( 'Метки %1$s', 'wp-bootstrap-starter' ) . '</span>', $tags_list ); // WPCS: XSS OK.
		}
	}
